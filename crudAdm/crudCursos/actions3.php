    ...
    <div class="container">
        ...
        <div class="card mb-3" id="moldura">
          ...
          function excluir(){
            ...
            //include('C:\xampp/htdocs/ProjTCC/crudAdm/crudCursos/conexaoBD.php');

            //$id = $_GET['rowid']; //obtém a linha
            $nome = $_POST["nome"];

            /*$resultado = mysqli_query($conexao, "SELECT * FROM cursos where nome = " . $nome);
            echo "aquiiiiii". $resultado;
            while ($row = mysqli_fetch_array($resultado)) {
              $id = $row['id'];
            }
            $id = mysqli_query($conexao, "SELECT id FROM cursos where nome = " . $nome);
            //$sql = "SELECT id FROM cursos where nome = " . $nome;
            //echo $id . "<br>";
            $sql = "DELETE FROM cursos WHERE id = " . $id; // montar string de conexão com o id concatenado.
            ...
            if ($linhas == 1) {
              echo "<p align=\"center\" id=\"texto2\">Curso " . $nome . " foi excluído com sucesso!</p>";
                          
            } else {
              echo "<p align=\"center\" id=\"texto2\">Não foi possível excluir o curso " . $nome . "!<br> 
              Verifique se esse curso está inserido no sistema! " . "</p>";
            }
            //echo "<br>";

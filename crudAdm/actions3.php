...
<body style="background-size: 100%; ">
  ...
  <?php if ($tela <= 767) { ?>
        ...
        <?php } ?>
			<!--<div style="position: absolute; color: #424242; text-align: center; top: 50%; left: 50%; margin-right: -50%; transform: translate(-50%, -50%) ">
  -->	<?php
				//session_start();
          ...
          $select = "SELECT id FROM usuario WHERE matricula = '$matricula'";
          $result_select = mysqli_fetch_assoc(mysqli_query($conexao, $select));
          $id = $result_select['id'];

					$sql = "DELETE FROM usuario WHERE id = " . $id; // montar string de conexão com o id concatenado.
					...
					if ($linhas == 1) {
            echo "<p align=\"center\" id=\"texto2\">Registro excluído com sucesso!</p>";
                        
					} else {
            echo "<p align=\"center\" id=\"texto2\">Não foi possível excluir! <br> Matricula não encontrada. <br></p>";
